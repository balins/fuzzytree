...
from ._fuzzy_tree import FuzzyRegressionTree, FuzzyTree, FuzzyTreeBuilder
...
class BaseFuzzyDecisionTree(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None, check_input=True):
        ...
        if y.ndim != 1:
            raise ValueError(
                "Multi-output problems are not currently supported (got y.ndim=%s)."
                % y.ndim
            )
        self.n_outputs_ = 1
        ...
        if not self.min_membership_leaf > 0:
            raise ValueError(
                "min_membership_leaf must be greater than 0.0, got %s"
                % self.min_membership_leaf
            )

        if not self.min_membership_split > 0:
            raise ValueError(
                "min_membership_split must be greater than 0.0, got %s"
                % self.min_membership_split
            )

        min_membership_split = max(
            self.min_membership_split, 2 * self.min_membership_leaf
        )
        ...
        splitter = Splitter(
            CRITERIA_CLF[self.criterion], self.fuzziness, self.min_membership_leaf
        )
